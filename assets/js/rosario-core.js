export class rosario {
  ...
  secuencia = ['Primer', 'Segundo', 'Tercero', 'Cuarto', 'Quinto'];
  ...
  pasos = [
    ...
    {
      end: ['Letanías', 'Cordero de Dios', 'Oración', 'Música'],
    },
  ];

  getMisterio() {
    const today = new Date().getDay();
    return Object.values(this.misterios).find((m) =>
      m.days.join().includes(today)
    );
  }
  ...
  duplicateArray = (_array, times) => {
    ...
    for (let i = 0; i < times; i++) {
      ...
      const misterioIndex = _newArray.findIndex((text) =>
        text.includes('N misterio')
      );
      _newArray[misterioIndex] = `${this.secuencia[i]} misterio: ${
        this.getMisterio().title[i]
      }`;
  ...
  renderRosario = () =>
    Object.values(this.pasos)
      .map((paso) => {
        if (Object.keys(paso).includes('repeat'))
          return this.duplicateArray(paso.repeat, 5);
  ...
  emulate = (action) => {
    ...
    if (action === 'before' && this.indexRosario > 0) {
      this.indexRosario--;
    }
...
class drawRosario {
  ...
  createRosario(numberSteps) {
    ...
    for (let i = 0; i < isContainerDouble; i++) {
      this.createContainers(rosarioEl, numberSteps / isContainerDouble);
    }
  }
